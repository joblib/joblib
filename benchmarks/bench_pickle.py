...
def generate_rand_dict(
    size, with_arrays=False, with_string=False, array_shape=(10, 10)
):
...
def print_line(
    dataset, strategy, write_time, read_time, mem_write, mem_read, disk_used
):
    """Nice printing function."""
    print(
        "% 15s, %12s, % 6.3f, % 7.4f, % 9.1f, % 9.1f, % 5.1f"
        % (dataset, strategy, write_time, read_time, mem_write, mem_read, disk_used)
    )
...
""".format(
            args.size,
            "np.ndarray" if args.valuearray else "str" if args.valuestring else "int",
        )
        if args.valuearray:
...
def bench_compress(dataset, name="", compress=("zlib", 0), cache_size=0, tries=5):
    ...
    if isinstance(compress, tuple) and tuple(
        map(int, joblib.__version__.split(".")[:2])
    ) < (0, 10):
        compress = compress[1]

    time_write = time_read = du = mem_read = mem_write = []
    clear_out()
    time_write, obj = timeit(
        joblib.dump,
        dataset,
        "out/test.pkl",
        tries=tries,
    ...
    del obj
    gc.collect()
    mem_write = memory_used(
        joblib.dump, dataset, "out/test.pkl", compress=compress, cache_size=cache_size
    )
    delete_obj(dataset)
...
def bench_mmap(dataset, name="", cache_size=0, mmap_mode="r", tries=5):
    ...
    clear_out()
    time_write, _ = timeit(
        joblib.dump, dataset, "out/test.pkl", tries=tries, cache_size=cache_size
    )
    mem_write = memory_used(joblib.dump, dataset, "out/test.pkl", cache_size=cache_size)

    delete_obj(dataset)

    time_read, obj = timeit(
        joblib.load, "out/test.pkl", tries=tries, mmap_mode=mmap_mode
    )
    delete_obj(obj)
    mem_read = memory_used(joblib.load, "out/test.pkl", mmap_mode=mmap_mode)
    du = disk_used("out") / 1024.0
    print_line(
        name, "mmap %s" % mmap_mode, time_write, time_read, mem_write, mem_read, du
    )
            ...
            for name, nifti_file in (
                ...
                for c_order in (True, False):
                    ...
                    if not args.nommap:
                        ...
                        run_bench(
                            bench_mmap, d, name_d, mmap_mode=mmap_mode, tries=args.tries
                        )
                        del d
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of tries to compute execution timemean on.",
    )
    parser.add_argument(
        "--shape", nargs="+", type=int, default=(10000, 10000), help="Big array shape."
    )
    parser.add_argument(
        "-m", "--nommap", action="store_true", help="Don't bench memmap"
    )
    ...
    parser.add_argument(
        "-n", "--nifti", action="store_true", help="Benchmark Nifti data"
    )
    parser.add_argument(
        "-a", "--array", action="store_true", help="Benchmark single big numpy array"
    )
    parser.add_argument(
        "-A", "--arrays", action="store_true", help="Benchmark list of big numpy arrays"
    )
    parser.add_argument(
        "-d", "--dict", action="store_true", help="Benchmark big dictionary."
    )
    ...
    parser.add_argument(
        ...
        "--combo",
        action="store_true",
        help="Benchmark big dictionary + list of big numpy arrays.",
    )
    parser.add_argument(
        "-s", "--summary", action="store_true", help="Show bench summary."
    )
