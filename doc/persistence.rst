...
save space on disk:
  ...
  >>> joblib.dump(to_persist, filename + '.z', compress=True)  # doctest: +ELLIPSIS
  ['...test.pkl.z']

By default, `joblib.dump` uses the zlib compression method as it gives the best
tradeoff between speed and disk space. It also supports other compression
formats such as 'gzip', 'bz2', 'lzma' and 'xz':
