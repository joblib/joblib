...
This example illustrates how to use a ``tqdm`` progress bar along with
:class:`joblib.Parallel`.

We will see two main ways to do so. The first one,
very simple, can be used when using a generator as the output of a
:class:`~joblib.Parallel` call. When the output is not a generator (*e.g.*
when using ``return_as='list'``), we will have to define a subclass of
:class:`~joblib.Parallel`.
...
##############################################################################
# We will first define a task that will sleep for a time given as a parameter
...
# the following example, this is not convenient, as we have to wait for all
...
##############################################################################
# As a result, the progress bar only appears after all tasks are completed,
...
##############################################################################
# The progress bar seems to work now. But we can still observe that it jumps
# from 1/7 to 4/7. This is because when using ``return_as='generator'``, the
# outputs are returned in the same order the tasks were given. So the first
# value is the output of the task taking 0.7 s
# (remember ``times = [7, 2, 3, 5, 6, 4, 1]``). When the first task is
# completed, the second and third tasks should also have completed, as
# 2+3 < 7. That's why we jump from 1/7 to 4/7.
#
# If you don't care about the order of the outputs and want better progress
...
##############################################################################
# The progress bar now advances one iteration at a time. Observe also how the
# order of the outputs changed in the last line.
...
# ``tqdm``, but we must rely on a new subclass of :class:`~joblib.Parallel`.
# The :class:`~joblib.Parallel` class has a method
# :meth:`~joblib.Parallel.print_progress`, which is called each time a task is
# completed. This function originally prints more or less information depending
